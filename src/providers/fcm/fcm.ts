...
export class FcmProvider {
  ...
  monitorTokenRefresh() {
    if (this.platform.is('cordova')) {
      return this.firebaseNative.onTokenRefresh().pipe(
        tap(token => this.saveTokenToFirestore(token) )
      )
    } else {
      // PWA implementation
      return of(null)
  ...
  async subscribeTo(topic: string) {
    ...
    return this.afs.collection('users').doc(user.uid).set({ topics }, { merge: true})
  }
